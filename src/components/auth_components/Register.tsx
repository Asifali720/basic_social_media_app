...
const validationSchema: Yup.Schema<FormikValues> = Yup.object().shape({
  ...
  email: Yup.string().required('email is required...!').email('please put valid email'),
  password: Yup.string().required('password is required...!').min(6, 'your password must be at least 6 character').matches(/^[a-zA-Z0-9!@#$%^&*()_+[\]{};':"\\|,.<>/?-]+$/, 'Password can only contain letters, numbers, and special characters'),
...
<h3 className="font-roboto font-bold text-2xl mb-6">Register now..!</h3>
      <form onSubmit={formik.handleSubmit}>
      <div className="flex flex-col items-start mb-2">
          <label htmlFor="name" className="text-gray-600 font-roboto text-sm font-medium mb-1">Username</label>
          <input
            id="username"
            name="username"
            type="username"
          ...
          {formik.touched.email && formik.errors.email ? (
            <div className="text-red-600 text-xs font-roboto mt-[2px]">{formik.errors.username}</div>
          ) : null}
        </div>
        <div className="flex flex-col items-start mb-2">
        ...
        <div className="flex flex-col items-start mb-12">
          <label htmlFor="confirmPassword" className="text-gray-600 font-roboto text-sm font-medium mb-1">Confirm Password</label>
          <div className="w-full bg-white flex items-center border-2 border-sky-500/50 rounded-sm  focus:border-sky-500">
          <input
            id="confirmPassword"
          ...
          {formik.touched.confirmPassword && formik.errors.confirmPassword ? (
            <div className="text-red-600 text-xs font-roboto mt-[2px]">{formik.errors.confirmPassword}</div>
          ) : null}
        </div>
        <Button type="submit" className="w-full">Register</Button>
      </form>
      <span className="block w-full text-center text-sm font-roboto font-medium my-5">or</span>
      <Button  className="w-full flex items-center gap-2"><img src={googleIcon} alt="icon" className="h-5 w-5"/><p>Register with Google</p></Button>
    </div>
  )
}
